...
class GeneralStar:
    def __init__(
        ...
        #self.midplane = np.random.uniform(low=-np.pi/2, high=np.pi/2) # this is wrong! isotropic inclinations requires uniformity in cos i space, not i!
        self.midplane = np.arcsin(np.random.uniform(low=-1, high=1)) # using arcsin instead of arccos bc of my chosen convention
...
class K2Star:
    ...
    def __init__(
        ...
        if self.num_planets!=None:
            ...
            sigma = self.sigma_incl
            self.incls = np.random.normal(loc=mu, scale=sigma, size=self.num_planets)
            #self.incls = np.arcsin(np.random.uniform(low=-1, high=1) + ) np.random.normal(loc=mu, scale=sigma, size=self.num_planets)
...
class StarZ23:
    ...
    def __init__(
        self, age, stellar_radius, stellar_mass, Teff, rrmscdpp06p0, height, alpha_se, alpha_sn, beta1_se, beta1_sn, beta2_se, beta2_sn, pbr_se, pbr_sn, frac_host, prob_intact, GaiaDR3=None, Kepler_ID=None, **kwargs 
    ):
        self.GaiaDR3 = GaiaDR3
        self.Kepler_ID = Kepler_ID
        self.age = age
        ...
        self.alpha_se = alpha_se
        self.alpha_sn = alpha_sn
        self.beta1_se = beta1_se
        self.beta1_sn = beta1_sn
        self.beta2_se = beta2_se
        self.beta2_sn = beta2_sn
        self.pbr_se = pbr_se
        self.pbr_sn = pbr_sn
        #self.kepid = np.array(kepid)
        #print(self.kepid, self.stellar_radius, self.stellar_mass, self.rrmscdpp06p0, self.height)
        ...
        if self.num_planets!=None:
            ...
            ### draw planet radii and periods such that they satisfy a Hill radius check and satisfy population statistics of radius valley. also draw planet masses.
            self.periods, self.planet_radii, self.planet_masses, self.se_or_sn = simulate_helpers.draw_planet_periods_and_radii_z23(self.num_planets, self.alpha_se, self.alpha_sn, self.beta1_se, self.beta1_sn, self.beta2_se, self.beta2_sn, self.pbr_se, self.pbr_sn, self.stellar_mass)
...
class K2StarZ23:
    ...
    def __init__(
        self, age, stellar_radius, stellar_mass, Teff, rrmscdpp06p0, height, alpha_se, alpha_sn, beta1_se, beta1_sn, beta2_se, beta2_sn, pbr_se, pbr_sn, frac_host, prob_intact, campaign, baseline, GaiaDR3=None, EPIC_ID=None, logistic_a=0.6095, logistic_k=0.6088, logistic_l=10.8986, **kwargs 
    ):
        self.EPIC_ID = EPIC_ID
        self.GaiaDR3 = GaiaDR3
        self.age = age
